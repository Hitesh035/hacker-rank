package hackerrank.algorithms.implementation.matrixLayerRotationIterationThree;
...
//			}
		...
		for (int layer = 1; layer <= ((rows >= columns) ? columns : rows) / 2; layer++) {
			...
			if (layerHoppingPosition == 0)
				continue;
			int hopperCount = layerHoppingPosition;
			for (int elements = 1; elements <= currentLayerElements; elements++) {

				for (int rotations = 1; rotations <= layerHoppingPosition; rotations++) {
