package hackerrank.algorithms.implementation.matrixLayerRotationIterationTwo;
...
//			}
		...
		for (int layer = 1; layer <= ((rows >= columns) ? columns : rows) / 2; layer++) {
			...
			int layerHoppingPosition = r > currentLayerElements ? (r % currentLayerElements) : (r);
			if (layerHoppingPosition == 0)
				continue;
			int hopperCount;
			...
			for (int elements = 1; elements <= currentLayerElements; elements++) {
				...
				for (hopperCount = layerHoppingPosition; hopperCount >0; hopperCount--) {
					if (k + 1 <= layerMaxRowIndex && l == layerMinColumnIndex) {
						resultArray[k + 1][l] = currentRotationArray[i][j];
						k++;
					} else if (k + 1 > layerMaxRowIndex && l + 1 <= layerMaxColumnIndex) {
						resultArray[k][l + 1] = currentRotationArray[i][j];
						l++;
					} else if (l + 1 > layerMaxColumnIndex && k - 1 >= layerMinRowIndex) {
						resultArray[k - 1][l] = currentRotationArray[i][j];
						k--;
					} else if (k - 1 < layerMinRowIndex && l - 1 >= layerMinColumnIndex) {
						resultArray[k][l - 1] = currentRotationArray[i][j];
						l--;
					}
				}
